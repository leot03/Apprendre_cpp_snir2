...
ServeurBanque::ServeurBanque(QObject *parent) : QTcpServer(parent) {
  connect(this, &QTcpServer::newConnection, this,
          &ServeurBanque::onServeurBanque_newConnection);
}
...
void ServeurBanque::Arreter() { close(); }
...
void ServeurBanque::onServeurBanque_newConnection() {
  while (hasPendingConnections()) {
    ...
    if (client != nullptr) {
      connect(client, &CompteClient::readyRead, this,
              &ServeurBanque::onCompteClient_readyRead);
      connect(client, &CompteClient::disconnected, this,
              &ServeurBanque::onCompteClient_disconnected);
...
void ServeurBanque::onCompteClient_readyRead() {
  ...
  if (socketVersClient->bytesAvailable() >=
      static_cast<qint64>(sizeof(taille))) {
    ...
    if (socketVersClient->bytesAvailable() >= (qint64)taille) {
      ...
      case 'R':
        ...
        if (operationPossible) {
          message = "nouveau solde " +
                    QString::number(socketVersClient->ObtenirSolde());
        } else {
