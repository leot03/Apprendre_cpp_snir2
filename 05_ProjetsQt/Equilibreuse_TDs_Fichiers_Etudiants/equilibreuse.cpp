...
Equilibreuse::Equilibreuse(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::Equilibreuse), experience(nullptr),
      vueGraphique(nullptr), chartView(nullptr) {
  ui->setupUi(this);
...
Equilibreuse::~Equilibreuse() { delete ui; }
...
void Equilibreuse::on_actionImporter_mesures_brutes_triggered() {
  ...
  leFichier = QFileDialog::getOpenFileName(
      this, "Ouverture fichier mesures brutes",
      "/home/USERS/ELEVES/SNIR2021/lthomas/", "Fichier mesure brute(*.mbr)");
 ...
 * @details Affiche le tableau d'acquisition à partir des données de
 * l'expérience. Elle deux courbes doivent-être initialisée ou chargée.
 */
void Equilibreuse::MettreAjourTableauAcquisition() {
  if (experience != nullptr) {
    QPointF point;
    point = experience->RechercherEffortMax('A');
    ui->tableWidgetAcquisition->item(0, 0)->setText(
        QString::number(point.y(), 'f', 1));
    ui->tableWidgetAcquisition->item(0, 1)->setText(
        QString::number(point.x(), 'f', 0));
    point = experience->RechercherEffortMax('O');
    ui->tableWidgetAcquisition->item(1, 0)->setText(
        QString::number(point.y(), 'f', 1));
    ui->tableWidgetAcquisition->item(1, 1)->setText(
        QString::number(point.x(), 'f', 0));
    double valeur;
    valeur = experience->RechercherEffortAzero('A');
    ui->tableWidgetAcquisition->item(0, 2)->setText(
        QString::number(valeur, 'f', 1));
    valeur = experience->RechercherEffortAzero('O');
    ui->tableWidgetAcquisition->item(1, 2)->setText(
        QString::number(valeur, 'f', 1));
    valeur = experience->RechercherEffortAquatreVingtDix('A');
    ui->tableWidgetAcquisition->item(0, 3)->setText(
        QString::number(valeur, 'f', 1));
    valeur = experience->RechercherEffortAquatreVingtDix('O');
    ui->tableWidgetAcquisition->item(1, 3)->setText(
        QString::number(valeur, 'f', 1));
  }
}
